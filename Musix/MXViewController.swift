...
class MXViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    @IBAction func addSong(_ sender: Any) {
        ...
        var allItems: [SongList] = []

        dialog.beginSheetModal(for: self.view.window!) { [self] result in
            if result == .OK {
                if let url = dialog.url {
                    let newSong = SongList(name: "song", file: url.path)
                    let encoder = JSONEncoder()
                    let jsonURL = Bundle.main.url(forResource: "songlist", withExtension: "json")
                    allItems.append(contentsOf: songList!)
